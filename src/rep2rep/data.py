...
class Rep2RepDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_volume = images[0, start:start + depth, region[0], region[1]]
        target_volume = images[1, start:start + depth, region[0], region[1]]
        reference = images[:, start:start + depth, region[0], region[1]].mean(axis=0)
        if sigma.ndim == 4:
            input_sigma = sigma[0, start:start + depth, region[0], region[1]]
        else:
            input_sigma = sigma[start:start + depth, region[0], region[1]]
        return {
